...
class Environment
  ...
  class Resource
    ...
    attr_reader :uri
    attr_reader :rootpackage
    def initialize(name, filename, uri = nil, rootpackage = nil)
      @name = name
      @filename = filename
      @uri = uri
  ...
  def initialize
    ...
    @deleted = {}
    @deletedClasses = {}
    @resourceSet = [Resource.new(
        "ecore",
        nil,
        "http://www.eclipse.org/emf/2002/Ecore",
        RGen::ECore::EObject.ecore.ePackage
    )]
  end

  def resourceAdd(name, filename, uri, rootpackage = nil)
    @resourceSet += [Resource.new(name, filename, uri, rootpackage)]
  end

  def contextFor(uri, currentRootPackageName)
    # When the URI is empty, use the current root package
    uri = currentRootPackageName if (uri.nil? || uri.empty?)
    resource = @resourceSet.find{|r|
      r.filename == uri ||
      r.uri == uri
    }
    resource.rootpackage unless resource.nil?
  end
  ...
  # Models are tree structures, so elements without parent/containers are root;
  # This is especially useful when loading multiple ecore files into the environment.
  def rootPackage(name)
    elements.find { |e| e.eContainer.nil? && e.name == name }
  end
